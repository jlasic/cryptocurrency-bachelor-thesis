...
class Node{
    constructor(port = 3001, blockchain, peers = []){
        ...
        this.wallet = new Wallet('jaaa');
        this.miner = new Miner(blockchain, this.wallet);
    ...
    initListeners(){
        ...
        this.miner.on('blockFound', (block) => {
            console.log(`block ${block.index}. found: ${block.hash} `);
            //broadcast block;
        });
    ...
    addTransaction(transaction){
        ...
        //checkTransaction();
        if (!Transaction.wasTempered(transaction))
            if(this.blockchain.isUTXO(transaction.inputs[0].outputTxId, transaction.inputs[0].outputIndex))
                // more checks missing 
                // sum(inputs) < sum(outputs)
                // pubkeyHash(output) === pubKey(input)
