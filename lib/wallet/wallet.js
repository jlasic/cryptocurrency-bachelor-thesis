...
class Wallet{
    constructor(password, blockchain){
       ...
       // TODO restore previous wallet (all of the previous addresses)
        blockchain.on('blockAdded', function added(block){
            this._checkAvailableOutputs(block);
        }.bind(this));

        blockchain.on('blockchainSwapped', function swapped(blocks){
            this.availableOutputs = [];
    ...
    */
    createTransaction(destAddress, amount){
        const fundsNeeded = amount + config.minimumFee;
    ...
    */
    _signTransaction(transaction){
        ...
        let signed = transaction.inputs.every(input => {
            ...
            let matchingKeyPair = this.keyPairs.find(keyPair => {
    ...
    _checkAvailableOutputs(block){
        block.transactions.forEach(transaction => {
            ...
            transaction.inputs.forEach(input => {
                this.availableOutputs = this.availableOutputs.filter(utxo => {
